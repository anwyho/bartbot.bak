...
from compose.receive.message import (Coordinate, Message, safe_parse)
from compose.receive.message import (ParamType)
...
class Payload():
    ...
    def __init__(self, attachment: dict, text: str):
        ...
        self.text: Optional[str] = text if self.attmType == 'FALLBACK' else None
        self.title: Optional[str] = attachment['title'] if self.attmType == 'FALLBACK' else None
...
class Attachment(Message):
    SUPPORTED_ATTACHMENT_TYPES = ['IMAGE', 'VIDEO', 'AUDIO', 'FILE', 'LOCATION',
                                  'FALLBACK']
