...
from compose.send import (set_if_exists)
from compose.send.button import (Button, UrlButton)
...
class GenericTemplate(Template):
    ...
    def add_element(self, **elementArgs: Union[Optional[str], bool]) -> bool:
        if len(self._elements) > self.MAX_GENERIC_ELEMENTS:
            logging.warning(
                f"Attempted to add one too many elements to template. Max number of elements is {self.MAX_GENERIC_ELEMENTS}.")
            return False
        else:
...
class ButtonTemplate(Template):
    ...
    def __init__(self,
        ...
        if not isinstance(buttons, list):
            ValueError(
                "The buttons argument in ButtonTemplate must be a list of Buttons.")
        # Check that there are buttons in buttons
        if not buttons:
            ValueError(
                "There must be buttons in the buttons list argument of ButtonTemplate.")
...
class ListTemplate(Template):
    ...
    def build(self) -> dict:
        if len(self._elements) < self.MIN_LIST_ELEMENTS:
            raise ValueError(
                f"Attempted to build List Template without {self.MIN_LIST_ELEMENTS} list elements. Please add {self.MIN_LIST_ELEMENTS-len(self._elements)} to {self.MAX_LIST_ELEMENTS-len(self._elements)} more list elements to build this template.")
    ...
    def __init__(self,
        ...
        if not hasattr(self, '_url'):
            if attachmentId and attachmentId.isdigit():
                ...
                ValueError(
                    f"Invalid attachment ID '{attachmentId}' provided for media template.")
            if not hasattr(self, '_attachmentId'):
                ValueError(
                    "Attempted to make a media template without any media source. Please include a valid attachment ID or URL.")
...
class ShareTemplate(GenericTemplate):
    ...
    def add_element(self, **elementArgs: Union[Optional[str], bool]):
        raise NotImplementedError(
            "Adding more elements is not supported for making a new ShareTemplate.")
